# OmniRoute 免费模型渠道清单 v4（2026-08-26 07:30 更新）
...
| 2 | OpenRouter :free | **minimax-m3:free（新）** / nemotron-3-ultra:free / nemotron-3-super:free / gpt-oss-20b:free | 20 req/min/key | ✓ 已实测 | ✅ 已有 |
...
| # | 渠道 | 潜在模型 | 状态 | 备注 |
|---|------|---------|------|------|
| 4 | NIM 新旗舰 | ~~minimax-m3~~ / ~~kimi-k3~~ / ~~ds-v4-flash-0731~~ | **确认免费 key 不可用**（实测挂起 >60s，sync 仅 4 个 free 模型） | step-3.7-flash 已确认可用并接入 free-all；其余 3 个需付费/无权限，放弃 |
| 5 | OpenRouter :free | z-ai/glm-5.2:free | 目录有但仍 429 未解锁（08-26 复测） | NIM 通道已下架，此模型目前无免费可用通道 |
...
| nemotron-3.5-lightning:free (OR) | 08-26 恢复 200 但输出烂（reasoning 泄漏，~35 不达标），不接入 |
...
| 4 | opencode/nemotron-3.5-lightning-free | opencode | ~35 | ✅ 1.9s | 推理模型，轻量 |
| 5 | opencode/nemotron-3-ultra-free | opencode | ~45 | ✅ 3-4s | Nemotron 3 Ultra 550B（复测 3/3 过，首测 502 瞬时抖动） |
| 6 | openrouter/minimax/minimax-m3:free | openrouter | 44.4 | ✅ 2.6-3.8s | **08-26 新增**，OR :free 新上架 |
| 7 | nvidia/stepfun-ai/step-3.7-flash | nvidia | ~42 | ✅ 0.97-1.6s | **08-26 新增**，NIM 推理模型 |

**策略**：round-robin / weight=0 / maxRetries=2 / concurrencyPerModel=3
**结论**：**7/7 全部实测通过（v19），无移除**
...
## 本轮维护记录（2026-08-26 07:30）

### ✅ 新增接入（free-all v17→v19，5→7 模型）
- **openrouter/minimax/minimax-m3:free**（AAII 44.4）：OR :free 新上架，2/2 实测 200 / 2.6-2.8s / 答对
- **nvidia/stepfun-ai/step-3.7-flash**（AAII ~42）：手动触发 NIM provider sync 后确认可路由，3/3 实测 200 / 0.97-1.6s / 答对（max_tokens 小会截断 reasoning 属正常）

### 🔍 渠道扫描结论
- Dahl：仍 3 模型无新增；43 tokens 全 active（≥40 不补建）
- NIM：手动 POST /api/providers/{connId}/sync-models 触发同步 → 免费 key 实际可见仅 4 模型（gemma-4-31b-it 挂起 50s 不可用、nemotron-3-super-120b 可用但无 ELO 证据、gpt-oss-120b 43s 超时门槛、gpt-oss-20b 不达标）
- NIM minimax-m3 / kimi-k3 / deepseek-v4-flash-0731：目录有但**免费 key 实测挂起 >60s**（非 free tier），放弃接入，标记待付费/权限
- OpenRouter：17 个 :free；glm-5.2:free 仍 429；nemotron-3.5-lightning:free 恢复 200 但输出烂不达标

### ❌ 未移除
- opencode/nemotron-3-ultra-free 首测 502 瞬时抖动，复测 3/3 全 200，保留

### 📊 配额状态
- Dahl：43 tokens（≥40 目标）全 active；NIM：21 keys；OpenRouter：1 key
