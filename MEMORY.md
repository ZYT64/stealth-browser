...
- L1-L7 全层记忆：会话/日记/profile/长期/教训/语义（NIM）/wiki；Dreaming 03:00 自动沉淀
...
## 核心约束
- 大陆网络红线（08-04）：禁接被墙平台项目（TG/X/Etherscan/Google 等）；投标/选型前先测连通性
- 模型纪律（08-10）：**deepseek-v4-pro 任何场景禁用**（含 fallback/子 Agent/omniroute 路由）；复杂推理用 minimax-m3 / deepseek-reasoner
- **勿自作主张替用户做决定（08-11 教训）**：用户方案有深层原因（防限流/防墙/防追踪），恢复/简化/清理前先确认；不暴力压测限流

## ClawHunt 接单（2026-08-04 建立）
- Agent id 1146，key ~/.clawhunt/agent.env；每日 09:40/18:40 扫描 → bid → 交付（isolated cron）；**bid 单位 cents**（100=$1.00 投错不可撤）；投标 message ≤1000 字符（**标书按 ≤950 写防超限**）；**已投判断=CLI bid 报 400 "Agent already bid"=幂等已投**（08-24 修正：08-23 的「平台级屏蔽」系误判，17 单实为全部已投覆盖，非脏数据）；**bid 不可撤回，只能中标后 abandon**（agent.abandon(pid, reason)）；**bid 按 reference_price 8-9 折动态定价，禁 hardcode 100**（08-22 四单 $1 低价触碰废标红线）；中标看 me().active_task_count + solve 能否提交（clawhunt status 是全平台列表，勿被误导）；涉墙单不接中标后 abandon；空模板/指定承接方/样片单跳过；操作规范见 clawhunt-bidding-ops skill
...
- Obsidian：arm64 无头 ~/apps/obsidian，vault ~/Study，systemd + CLI 操作（细节 lessons.md/profile.md）

## 归档索引
- 历史 promotion 已全部分层归档：08-04/05（minimax 转述/DeepTutor/ClawHunt/NIM key/PIP）、08-06（Obsidian/PM+UX）、08-07（KuroTutor/FastContext）、08-10（v4-pro 禁用/summarize）、08-12/13（OmniRoute 防限流/502 排障/free-all/opencode 限流/stealth/批量邮箱，细节 TOOLS.md/lessons.md/profile.md/skill 提案）、08-14（DSH/免费模型挖掘 skill 提案）、08-15（opencode-web 服务/ClawHunt 空模板规则）、08-16（DeepSeek 欠费/free-all v9，见 memory/archive/2026-08-17.md）、08-17（ClawHunt 账号改进/probe 结论/升学咨询/凭据纪律，见 memory/archive/2026-08-18.md）、08-18（作文素材技能/免费池维护/ClawHunt 双扫，见 memory/archive/2026-08-19.md）、08-19（free-all v12/NIM 饱和/SSE 解析坑/ClawHunt 新单，见 memory/archive/2026-08-20.md）、08-20（free-all v13/combos 核对教训/ClawHunt #1276，见 memory/archive/2026-08-21.md）、08-21（主模型切 minimax/free-all v14/ClawHunt #1288，见 memory/archive/2026-08-22.md）、08-22/23（主模型换 deepseek-vision-exp/cron 统一 free-nvidia/ClawHunt $1 低价/用户作业偏好，见 memory/archive/2026-08-23.md）、08-23 晚（claude 卸载→opencode 主力/ClawHunt 平台屏蔽/OmniRoute 准入限流/Vibe Kanban 装卸/GPU 算力调研，见 memory/archive/2026-08-24.md）、08-24（opencode→Claude Code 回归/Stop Hook 长跑/TinyMind 数据回迁/Paperspace+百度 AI Studio 调研/ClawHunt「平台屏蔽」误判修正，见 memory/archive/2026-08-25.md）、08-25（combo 上下文误标 128K 修复/TinyMind 守护循环三修/OpenI 启智算力源选定+camoufox-cli 打通/晨报加自查自修维度，见 memory/archive/2026-08-26.md）；08-11/08-17/08-18 自动 promotion 重复已去重（原文在 memory/2026-08-10.md/2026-08-11.md/2026-08-13.md）；08-19 自动 promotion 5 条已压缩（内容沉淀至 profile/lessons/archive/2026-08-20.md）

## Promoted From Short-Term Memory (2026-08-26)

<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:4:7 -->
- free-model-maintain（07:30 cron）: **free-all 升级 v14（5→7 模型，round-robin）**：; **新增 NIM kimi-k3**（新旗舰，实测 1.0s；model_intelligence coding ELO 0.9389/1674，远超 dsv4flash 0.7265，为池内最强）→ 入池; **回补 opencode nemotron-3-ultra-free**（8-20 的 PUT 事故声称补回但实际不在池，今日全量核对发现缺失，实测 2.1s 正常）→ 入池; **NIM glm-5.2 恢复**（连续 3 日异常后今日 6.1s 正常作答）→ 保留 [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:4-7]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:8:11 -->
- free-model-maintain（07:30 cron）: 池内 7 模型逐个实测全 HTTP 200（0.6-6.6s，串行）：dahl DeepSeek-V4-Flash-0731 / Kimi-K2.6 / MiniMax-M2.7、opencode 3.5-lightning / ultra、NIM glm-5.2 / kimi-k3；整体 model=free-all 200(3.5s)；零移除; 经验：M2.7 / lightning / ultra 带 think 块，max_tokens=60 会被推理吃满导致 content 为空（非故障），测试需 ≥100; NIM 目录 103（+1）：kimi-k2.6 仍 404 无权限；nemotron-3-super-120b-a12b 实测可用（0.8s）但无 ELO 评分数据，Nemotron 系历史评分偏低，备选不入池; Dahl 目录仍 3 模型无新增（公开 catalog 免 key）；43 账户全 active 不补建；OpenRouter :free 仍 17 个无新增（glm-5.2:free 仍 429 不付费） [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:8-11]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:12:12 -->
- free-model-maintain（07:30 cron）: 无新 provider、无付费、无注册渠道；未使用 deepseek-v4-pro [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:12-12]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:15:18 -->
- ClawHunt 每日扫描（09:40 cron）: bidding 单总数：15（curl + SDK 双向验证 total=15, has_more=false）：1281、1280、1279、1278、1276、1245、1242、1239、1231、1222、1180、1172、1171、1096、1039; 本轮新投标 3 单（均已入 bids.json）：; **1281 汽车经销数智员工管理平台**：$8050（ref $9472 的 8.5 折），hard/ml，竞标 8 人，无 deadline; **1280 Amazon 商品内容生产 Agent V1**：$1258（ref $1480 的 8.5 折），hard/ml，竞标 9 人 [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:15-18]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:19:22 -->
- ClawHunt 每日扫描（09:40 cron）: **1279 亚马逊女装 AI 运营自动化**：$629（ref $740 的 8.5 折），hard/ml，竞标 9 人; 跳过：1278 SocialEye（涉墙-X，X API 需翻墙）、1245 铜梁文旅宣传片（需 demo_video_url 样片才能投标，SDK bid 无此字段，无法满足样片要求）、1096（描述为空模板）、1231（涉墙-WhatsApp，历史已跳过）; 仍在等待 8 单：1039（$0.85，58人）/1172（$1，48人）/1171（$1.2，40人）/1180（金额未记录，37人）/1222（$5，16人）/1239（$10，15人）/1242（$12，16人）/1276（$0.85，11人）; active_task_count=0，无中标任务，无需交付 [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:19-22]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:23:23 -->
- ClawHunt 每日扫描（09:40 cron）: 经验：1245 这类「带样片投标」的单子 SDK 的 bid(pid, amount, message) 没有 demo_video_url 字段，投不了，直接跳过并说明 [score=0.803 recalls=0 avg=0.620 source=memory/2026-08-21.md:23-23]
